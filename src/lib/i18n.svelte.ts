...
const DICT: Record<Lang, Record<string, Message>> = {
  zh: {
    ...
    'intro.jokersNote': Msg((p) => `整局只有 ${p.n} 次,点击按钮使用,用完不补。`),
    'intro.archive': '// 档案',
    'intro.best': '历史最高',
    ...
    'intro.footKeys': '键盘: A B C D 选项 · 点击按钮使用锦囊',
  ...
  en: {
    ...
    'intro.jokersNote': Msg((p) => `Only ${p.n} per run, use the buttons — no refills.`),
    ...
    'intro.footKeys': 'Keys: A B C D options · Use the buttons for jokers',
